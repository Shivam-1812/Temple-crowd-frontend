...
const ResourceAllocation = () => {
  const resources = [
    { name: 'Parking Slots', allocated: 80, total: 100, status: 'Active' },
    { name: 'Shuttle Buses', allocated: 6, total: 10, status: 'Active' },
    { name: 'Medical Stations', allocated: 3, total: 5, status: 'Active' },
    { name: 'Security Personnel', allocated: 20, total: 25, status: 'Active' }
  ]
  ...
  return (
    <div className="space-y-6">
          ...
          <tbody>
            {resources.map((resource, index) => (
              <tr key={index} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">{resource.allocated}</td>
                <td className="p-3">{resource.total}</td>
                <td className="p-3">
                  <span className="text-green-600 font-semibold">{resource.status}</span>
                </td>
                <td className="p-3">
      ...
      {/* Incoming Requests */}
      <div className="bg-white p-6 rounded-lg shadow-md">
        <h2 className="text-xl font-bold text-gray-800 mb-6 pb-2 border-b-2 border-orange-500">
